...
def main():
    ...
    print("{0} soubor{1}, {2} adresář{3}".format(
          "{0:n}".format(counts[0]) if counts[0] else "žádné",
          "ů" if counts[0] != 1 else "",
          "{0:n}".format(counts[1]) if counts[1] else "žádné",
          "ů" if counts[1] != 1 else ""))
...
def process_options():
    usage = """%prog [volby] [cesta1 [cesta2 [... cestaN]]]
...
Cesty jsou volitelné. Nejsou-li zadány, použije se aktuální adresář."""
    ...
    parser.add_option("-H", "--hidden", dest="hidden",
            action="store_true",
            help=("zobrazí skryté soubory [výchozí: off]"))
    parser.add_option("-m", "--modified", dest="modified",
            action="store_true",
            help=("zobrazí datum a čas poslední modifikace [výchozí: off]"))
    orderlist = ["name", "n", "modified", "m", "size", "s"]
    parser.add_option("-o", "--order", dest="order",
            choices=orderlist,
            help=("seřadí výstup podle ({0}) [výchozí: %default]".format(
                  ", ".join(["'" + x + "'" for x in orderlist]))))
    parser.add_option("-r", "--recursive", dest="recursive",
            action="store_true",
            help=("sestupuje rekurzivně  do podadresářů [výchozí: off]"))
    parser.add_option("-s", "--sizes", dest="sizes",
            action="store_true",
            help=("zobrazí velikosti [výchozí: off]"))

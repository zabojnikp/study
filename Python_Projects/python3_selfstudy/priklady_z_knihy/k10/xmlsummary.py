...
class Worker(threading.Thread):
    ...
    def process(self, filename):
        ...
        try:
            ...
            message = "obsahuje následující chybu:\n    " + str(err)
        except EnvironmentError as err:
            message = "nelze přečíst:\n    " + str(err)
        else:
            message = ("používá následující značky:\n    " +
                       "\n    ".join(sorted(tags, key=str.lower)))
        message = (self.number + filename + " je soubor XML, který " +
                   message)
...
def main():
    ...
    if opts.verbose:
        print("Vytvářím vlákna: {0}...".format(
              opts.count))
...
def parse_options():
    parser = optparse.OptionParser(
            usage=("použití: %prog [volby] [cesta]\n"
    ...
    parser.add_option("-t", "--threads", dest="count", default=7,
            type="int",
            help=("počet vláken, které se mají použít (1..20) "
                  "[výchozí %default]"))
    ...
    if not (1 <= opts.count <= 20):
        parser.error("počet vláken musí být 1..20")

...
def check_one_item(item):
    ...
    for filenames in md5s.values():
        if len(filenames) == 1:
            continue
        results.append("Duplicitní soubory ({0:n} bytes):\n\t{1}".format(
...
def parse_options():
    parser = optparse.OptionParser(
            usage=("použití: %prog [volby] [cesta]\n"
                   "vypíše seznam duplicitních souborů na zadané cestě "
                   "s použitím algoritmu MD5 \n"
                   "soubory s nulovou délkou ignoruje\n"
                   "výchozí hodnota cesty je ."))
...
if __name__ == "__main__": # Toto je ve Windows *nezbytné*!
    main()

...
class Bike:
    ...
    @name.setter
    def name(self, name):
        assert len(name), "název kola nesmí být prázdný"
        self.__name = name


    @property
    def quantity(self):
        "Množství těchto kol na skladě"
        return self.__quantity

    @quantity.setter
    def quantity(self, quantity):
        assert 0 <= quantity, "množství nesmí být záporné"
    ...
    def price(self, price):
        assert 0.0 <= price, "cena nesmí být záporná"
        self.__price = price


    @property
    def value(self):
        "Hodnota těchto kol na skaldě"
...
class BikeStock:
    ...
    change_name = lambda self, identity, name: self.__change_bike(
                                            identity, "name", name)
    change_name.__doc__ = "Změní název kola"

    change_price = lambda self, identity, price: self.__change_bike(
                                            identity, "price", name)
    change_price.__doc__ = "Změní cenu kola"
    ...
    increase_stock.__doc__ = ("Zvýší počet kusů zadaného kola na skladě "
                              "o zadané množství")

    decrease_stock = (lambda self, identity, amount:
                                self.__change_stock(identity, -amount))
    decrease_stock.__doc__ = ("Sníží počet kusů zadaného kola na skladě "
                              "o zadané množství")

...
def retrieve_car_details(previous_license):
    ...
    if not ok:
        print(data[0])
        while True:
            start = Console.get_string("Začátek SPZ", "SPZ")
            if not start:
                return previous_license, None
            start = start.upper()
            ok, *data = handle_request("GET_LICENSES_STARTING_WITH",
                                       start)
            if not data[0]:
                print("Žádná SPZ nezačíná na " + start)
                continue
            for i, license in enumerate(data[0]):
                print("({0}) {1}".format(i + 1, license))
            answer = Console.get_integer("Zadejte volbu (0 pro zrušení)",
    ...
    try:
        with SocketManager(tuple(Address)) as sock:
            ...
            if version > InfoVersion:
                raise ValueError("server není kompatibilní")
            result = bytearray()
            while True:
                data = sock.recv(4000)
                if not data:

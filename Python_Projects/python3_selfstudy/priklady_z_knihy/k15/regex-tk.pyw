...
class MainWindow:
    ...
    def __init__(self, parent):
        ...
        self.text.set("množství = 11.95, atd")
        ...
        regexLabel = tkinter.Label(frame, text="Reg. výraz:", underline=0)
        ...
        self.ignoreCaseCheckbutton = tkinter.Checkbutton(frame,
                text="Ignorovat velikost písmen", underline=0, variable=self.ignore_case)
        self.ignoreCaseCheckbutton.grid(row=2, column=0, columnspan=2,
                                        padx=2, pady=2, sticky=tkinter.E)
        self.dotallCheckbutton = tkinter.Checkbutton(frame, text="Tečka zahrnuje nové řádky",
                underline=15, variable=self.dotall)
        ...
        row = 3
        for i in range(10):
            label = tkinter.Label(frame, text="Skupina {0}".format(i))
        ...
        parent.bind("<Alt-n>", self.dotallChanged)
    ...
    def calculate(self, *ignore):
        for i in range(10):
            ...
            self.groupLabels[i]["text"] = "Skupina {0}".format(i)
        ...
        try:
            ...
            self.message.set("Neplatný regulární výraz")
            self.messageLabel["bg"] = "mistyrose"
        else:
            self.message.set("Nalezena shoda" if match is not None
                             else "Bez shody")

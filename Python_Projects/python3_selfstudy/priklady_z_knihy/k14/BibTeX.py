...
# Příklady jsou zkopírovány z Wikipedie
TEXT = """
...
def pyparsing_parse(text):
    ...
    try:
        ...
        print("Chyba při analýze: {0}".format(err))
    return books
...
def ply_parse(text):
    ...
    def t_error(t):
        ...
        print("nemohu analyzovat řádek {0}: {1}".format(t.lineno + 1,
                                                     line))
    ...
    for token in lexer:
        if token.type == "IDENTIFIER":
            ...
            continue
        if book is None:
            print("chybí řádek se začátkem knihy {0}".format(token.lineno))
        if token.type == "KEY":
            key = token.value
            continue
        if key is None:
            print("chybí klíčový řádek {0}".format(token.lineno))

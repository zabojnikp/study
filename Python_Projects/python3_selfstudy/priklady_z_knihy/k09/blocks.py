...
def recursive_descent_parse(text):
    ...
    Traceback (most recent call last):
    ...
    ValueError: Error {0}:line 1, column 1: očekáváno '[]/' obdrženo '#'
...
def pyparsing_parse(text):
    ...
    Traceback (most recent call last):
    ...
    ValueError: Error {0}: syntaktická chyba, line 8
    >>> filename = os.path.join(dirname, "error2.blk")
    >>> pyparsing_parse(open(filename, encoding="utf8").read())
    Traceback (most recent call last):
    ...
    ValueError: Error {0}: syntaktická chyba, line 1
    >>> filename = os.path.join(dirname, "error3.blk")
    >>> pyparsing_parse(open(filename, encoding="utf8").read())
    Traceback (most recent call last):
    ...
    ValueError: Error {0}: syntaktická chyba, line 4
    >>> expected = "[white: ]\\n[lightblue: Director]\\n/\\n/\\n[white: ]\\n[lightgreen: Secretary]\\n/\\n/\\n[white: Minion #1]\\n[white: ]\\n[white: Minion #2]"
    >>> filename = os.path.join(dirname, "hierarchy.blk")
...
def ply_parse(text):
    ...
    Traceback (most recent call last):
    ...
    ValueError: Error {0}:line 2: syntaktická chyba
    >>> filename = os.path.join(dirname, "error3.blk")
    >>> ply_parse(open(filename, encoding="utf8").read())
    Traceback (most recent call last):
    ...
    ...
    def t_error(t):
        ...
        raise LexError("syntaktická chyba: {1}".format(line))
...
použití: %prog [volby] vstupní_soubor1 [vstupní_soubor2 [...]]

Čte jeden či více 'blokových' souborů (.blk) s popisem a pro každý
zapíše ekvivalentní soubor .svg pro vizualizaci těchto bloků, 
při čemž použije stejný název, jako má vstupní soubor, 
ale odpovídajícím způsobem změní příponu.
""")
    ...
    if not len(args):
        parser.error("musí být zadán alespoň jeden soubor .blk")
    ...
    for file in files:
        ...
        try:
            ...
            if BlockOutput.save_blocks_as_svg(blocks, svg):
                print("Uloženo do {0}".format(svg))
            else:
                print("Chyba: nemohu uložit {0}".format(svg))

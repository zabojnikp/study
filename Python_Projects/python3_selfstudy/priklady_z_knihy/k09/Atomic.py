...
"""
Atomic je správce kontextu pro měnitelné kolekce.

Atomic zajišťuje, že se aplikují buď všechny změny nebo žádná z nich
(tj. v případě, že dojde k výjimce). Atomic vytvoří soukromou kopii
kolekce (hloubkovou kopii, má-li příznak deep_copy hodnotu True), což
může být pro rozsáhlé kolekce drahé.
...
>>> class A:
...     pass
>>> collections.MutableSet.register(A) # Tato třída lže o svém rozhraní API
>>> a = A()
>>> with Atomic(a) as atomic:
...     a.add(1)
Traceback (most recent call last):
...
AssertionError: Atomické mapování/množiny musí poskytovat metody clear() a update()
...
Traceback (most recent call last):
...
AssertionError: Atomic vyžaduje měnitelnou kolekci
"""
...
class Atomic:
    ...
    def __init__(self, container, deep_copy=False):
        assert (isinstance(container, collections.MutableSequence) or
                ...
                "Atomic vyžaduje měnitelnou kolekci")
        # if __debug__ ensures this is not executed when optimized with -O
        if (__debug__ and
            isinstance(container, collections.MutableSet) or
            isinstance(container, collections.MutableMapping)):
            assert (hasattr(container, "clear") and
                    hasattr(container, "update")), ("Atomické "
                    "mapování/množiny musí poskytovat metody clear() a update()")

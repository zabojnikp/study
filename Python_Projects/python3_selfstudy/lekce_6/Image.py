...
class Image:
    ...
    def __getitem__(self, coordinate):
        assert len(coordinate) == 2, "souradnice musi byt n-tice se dvema prvky"
    ...
    def export(self, filename):
        if filename.lower().endswith(".txt"):
            self.__export_txt(filename)
        else:
            raise ExportError("nepodporovany format pro export: " + os.path.splitext(filename[1]))
    
    def __export_txt(self, filename):
        """Exportuje obrázek jako soubor txt, pokud používá méně než 
        8930 barev
        """
        ...
        chars.reverse()
        if count > len(chars):
            raise ExportError("nemohu exportovat do souboru txt: příliš mnoho barev")
        fh = None
        try:
...
txt = os.path.join(tempfile.gettempdir(), "test.txt")
border_color = "#FF0000"
square_color = "#0000FF"
width, height = 240, 60
